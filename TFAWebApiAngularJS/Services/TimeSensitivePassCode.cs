...
namespace TFAWebApiAngularJS.Services
{
    public static class TimeSensitivePassCode
    {
        ...
        {
            //genera una chiave random di tipo stringa...
            byte[] key = new byte[10]; // 80 bits
            using (var rngProvider = new RNGCryptoServiceProvider())
            {
                rngProvider.GetBytes(key);
            }
            //Why did we encod the key using base32 format? Because Google Authenticator uses the same 
            //encoding to help end users who prefer to enter the Preshared key manually without any confusion
            return key.ToBase32String();
        }
        //implementation of Google Authenticator algorithms (HOTP, and TOTP)
        public static IList<string> GetListOfOTPs(string base32EncodedSecret)
        {
            //generating the One time passcodes
            DateTime epochStart = new DateTime(1970, 01, 01, 0, 0, 0, 0, DateTimeKind.Utc);

            long counter = (long)Math.Floor((DateTime.UtcNow - epochStart).TotalSeconds / 30);
            var otps = new List<string>();
            //three time sensitive pass codes, one in the past, another in the present, and one in the future; 
